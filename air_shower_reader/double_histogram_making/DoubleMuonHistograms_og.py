...
DoubleMuonMask=np.load('/data/user/vbasu/SelfVetoArrays/PickleFiles/DoubleMuonMask.npy',mmap_mode='r',allow_pickle=True)
...
print('Energy:',np.shape(Energy_L1_muon))
...
for multmask in MultiplicityMasks:
    ...
    if np.sum(ADmask)>0:
        ...
        stackarray=np.stack((Energy_L1_muon[ADmask],Energy_L2_muon[ADmask],Energy_Shower_Neutrino[ADmask]),axis=1)
        Hist3D,edges=np.histogramdd(stackarray,bins=(E_mu_bins,E_mu_bins,E_nu_bins),weights=Weight_Gaisser_corsika[ADmask])
