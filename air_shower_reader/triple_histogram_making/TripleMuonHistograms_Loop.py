        ...
        for depth in depths:
            ...
            for multmask in MultiplicityMasks:
                ...
                if np.sum(ADmask)>0:
                    ...
                    filename = '/home/zrechav/scripts/air_shower_reader/triple_histogram_making/histograms/' + flavour + '_Triple_Zen_' + str(np.around(angle,2)) + '_Depth_' + str(np.around(depth,2)) + '.npy'     

                    print('Zen_'+str(angle)+'_Depth_'+str(depth)+' '+str(np.sum(Hist4D)))
