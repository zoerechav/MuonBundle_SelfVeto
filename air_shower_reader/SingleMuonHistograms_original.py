...
SingleMuonMask=np.load('/data/user/vbasu/SelfVetoArrays/PickleFiles/SingleMuonMask.npy',mmap_mode='r',allow_pickle=True)
...
print('length mult mask:',len(MultiplicityMasks))
for multmask in MultiplicityMasks:
    ...
    if np.sum(ADmask)>0:
        print('I AM IN HERE') 
        Hist2D_trans,xedges,yedges = np.histogram2d(Energy_Shower_Neutrino[ADmask],Energy_L1_muon[ADmask],bins = [E_nu_bins,E_mu_bins],weights = Weight_Gaisser_corsika[ADmask])
